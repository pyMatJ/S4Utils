...
f = np.array([1]) ## single frequency in reduced units
a = 1 ## period
...
#%%
Rtm = np.empty((len(theta),len(f)))
Ttm = np.zeros_like(Rtm)
Atm = np.zeros_like(Rtm)


for ii, thi in enumerate(theta):
    for jj, fj in enumerate(f):
        S.SetFrequency(fj)
        ...
        Rtm[ii,jj] = np.abs(-r/inc)
        Ttm[ii,jj] = np.abs(fw/inc)
        fw1, bw1 = S.GetPowerFlux('Slab', 0)
        fw2, bw2 = S.GetPowerFlux('Slab', SlabThick)
        Atm[ii,jj] = np.abs((fw2-fw1-(bw1-bw2))/inc)
...
ax.plot(theta, Atm, 'o', fillstyle='none', label='A')
